...
float average(Student *pStudent, int total) {
    float sum = 0;
    for (int i = 0; i < total; i++) {
        sum += pStudent[i].grade;
    }
    return sum / total;
}
...
int main(int argc, char *argv[]) {
    ...
    float avg = average(pStudent, total);

    for (int i = 0; i < total; i++) {
        if (pStudent[i].grade > avg && strcmp(pStudent[i].programme, "CSCE") == 0) {
            printf("%s, %.2f\n",
            pStudent[i].name,
            pStudent[i].grade);
        }
